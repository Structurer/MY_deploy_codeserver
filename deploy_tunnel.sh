...
# 部署脚本：仅部署 Cloudflare Tunnel（转发到自定义本地端口）
# 作者：Trae Assistant
# 日期：2026-08-03
...
echo ""
echo "4. 安装 Cloudflare Tunnel 客户端..."
...
echo ""
echo "7. 配置完整域名..."
echo "请输入您要使用的完整域名（如 app.example.com）："
...
if [ -z "$TUNNEL_ID" ] && echo "$TUNNEL_CREATION_OUTPUT" | grep -q "tunnel with name already exists"; then
    ...
    if [ "$tunnel_choice" = "1" ]; then
        ...
        if [ -z "$NEW_TUNNEL_NAME" ]; then
            NEW_TUNNEL_NAME="custom-tunnel-$(date +%s)"
        fi
...
EOF
echo "已写入配置：转发 https://$FULL_DOMAIN -> http://localhost:$LOCAL_PORT"

echo ""
echo "10. 启动并启用 Cloudflare Tunnel 服务..."
# 检查服务是否已安装
...
echo ""
echo "11. 绑定自定义域名到 Cloudflare Tunnel..."
...
echo ""
echo "本地转发目标: http://localhost:$LOCAL_PORT"
echo ""
echo "=== 部署完成 ==="
